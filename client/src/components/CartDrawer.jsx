...
const WHATSAPP_PHONE = (import.meta.env.VITE_WHATSAPP_PHONE || '96171968846').replace(/\D/g, '');
...
export default function CartDrawer({ open, onClose }) {
  const { cartItems, total, updateQuantity, updateNote, removeItem, clearCart } = useCart();
  ...
  function validate() {
    ...
    if (customer.orderType === 'Delivery' && !customer.address.trim()) {
      nextErrors.address = 'Location/address is required for delivery.';
    }
  ...
  function handleOrder() {
    ...
    const message = buildWhatsAppMessage({ cartItems, customer, total });
        ...
        <div className="fixed inset-0 z-50" dir="ltr">
          ...
          >
            ...
            <div className="gold-scrollbar flex-1 overflow-y-auto px-4 py-4">
              {errors.cart ? (
                <div className="mb-3 rounded-md border border-red-400/35 bg-red-500/10 px-3 py-2 text-sm text-red-100">
                  {errors.cart}
                </div>
              ) : null}
              ...
              {cartItems.length ? (
                <div className="space-y-4">
                  {cartItems.map((item) => (
                    <article key={item.id} className="rounded-md border border-gold-500/20 bg-black/25 p-3">
                      ...
                      <div className="mt-3 flex items-center justify-between gap-3">
                        <div className="flex items-center overflow-hidden rounded-md border border-gold-500/25">
                          ...
                          <span className="flex h-9 min-w-12 items-center justify-center px-3 text-sm font-bold">
                            {item.quantity}
                          </span>
                          <button
                            type="button"
                ...
                <div className="space-y-3">
                  <label className="block">
                    ...
                    <input
                      ...
                      required
                    />
                    {errors.name ? <span className="mt-1 block text-xs text-red-200">{errors.name}</span> : null}
                  </label>
                    ...
                    <div className="grid grid-cols-2 overflow-hidden rounded-md border border-gold-500/25">
                      {['Pickup', 'Delivery'].map((type) => (
                        <button
                          ...
                          onClick={() => updateCustomer('orderType', type)}
                        >
                          {type}
                        </button>
                      ))}
                  ...
                  <label className="block">
                    ...
                    />
                    {errors.address ? <span className="mt-1 block text-xs text-red-200">{errors.address}</span> : null}
                  </label>
                </div>
              </section>
            </div>
